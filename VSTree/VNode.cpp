...
VNode::VNode()
{
    ...
    for(int i = 0; i < VNode::MAX_CHILD_NUM; i ++)
    {
        ...
        this->child_node_pointers[i] = NULL;
    }
}
...
bool VNode::addChildNode(VNode* _p_child_node, bool _is_splitting)
{
    ...
    this->addChildEntry( _p_child_node->getEntry(), _is_splitting); //increment child_num
...
void VNode::refreshAncestorSignature(LRUCache& _nodeBuffer)
{
    ...
    if (fatherNodePtr->getChildEntry(rank).getEntitySig() != this->entry.getEntitySig()) //@@: If father's child entry != updated child entry 
    { 
